...
class FPD_Catalog_V3_REST_API {
    ...
    public function register_routes() {
        register_rest_route( 'fpd-catalog-v3/v1', '/items', [
    ...
    public function get_items( \WP_REST_Request $request ) {
        ...
        $args = apply_filters( 'fpd_catalog_v3_query_args', $args );

        $cache_ttl = $request->get_param( 'cache_ttl' ) * MINUTE_IN_SECONDS;
        $cache_key = 'fpd_catalog_v3_' . md5( wp_json_encode( $args ) );

        $items = get_transient( $cache_key );

        if ( false === $items ) {
            $items = FPD_Data_Helper_V3::query_items( $args );
            if ( $cache_ttl > 0 ) {
